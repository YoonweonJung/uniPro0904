...
public class MemberList implements ShopreboardSuper{
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) {
		...
		//총게시물수 조회
		String sql1 = ReboardSQL.getSQL(ReboardSQL.SELECT_ROBOARDCNT);
		...
		String sql = ReboardSQL.getSQL(ReboardSQL.SELECT_REBOARDLIST);
		...
		try {
			...
			/*	vo.setWday(rs.getDate("wday"));
				...
				System.out.println(vo.getNo()+"/"+vo.getWriter()+"/"+vo.getTitle()
					                   +"/"+vo.getWday()+"/"+vo.getHit()+"/"+vo.getCnt());
			}
